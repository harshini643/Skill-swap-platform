...
def profile():
    ...
    try:
        payload = jwt.decode(token.replace('Bearer ', '', 1), 'secret', algorithms=['HS256'])
...
def search_skills():
    ...
    if not token:
        return jsonify({'error': 'Token required'}), 401
    try:
        jwt.decode(token.replace('Bearer ', '', 1), 'secret', algorithms=['HS256'])
...
def create_swap():
    ...
    try:
        payload = jwt.decode(token.replace('Bearer ', '', 1), 'secret', algorithms=['HS256'])
        data = request.get_json()
        if not all(key in data for key in ['responder_id', 'offered_skill_id', 'wanted_skill_id']):
            return jsonify({'error': 'Missing required fields'}), 400
...
def add_feedback():
    ...
    try:
        payload = jwt.decode(token.replace('Bearer ', '', 1), 'secret', algorithms=['HS256'])
        data = request.get_json()
        if not all(key in data for key in ['swap_id', 'rating']):
            return jsonify({'error': 'Missing swap_id or rating'}), 400
